...
def Processjson(data):
    """DEPRECATED - Use ProcessGandH instead"""
    json_data = []
    for project, tower, green, non_green, finishing in zip(
        data["Project Name"],
        data["Tower"],
...
def ProcessGandH(exceldatas, ignore_year, ignore_month):
    """
    Process Tower F, G, H slab data and return both JSON and DataFrame
    
    Returns:
        tuple: (json_string, dataframe)
        - json_string: JSON with format {"F": count, "G": count, "H": count}
        - dataframe: DataFrame with columns ['Tower', 'Completed']
    """
    wb = load_workbook(exceldatas)  
    sheet_name = "Revised Baselines- 25 days SC"
    sheet = wb[sheet_name]
    ...
    # Count green cells for each tower
    f_count = towerf.count(1)
    g_count = towerg.count(1)
    h_count = towerh.count(1)
    
    # Create JSON in the format expected by main code
    slab_dict = {
        "F": f_count,
        "G": g_count,
        "H": h_count
    ...
    json_data = json.dumps(slab_dict, indent=4)
    
    # Also create DataFrame for display purposes
    df = pd.DataFrame({
        'Tower': ['F', 'G', 'H'],
        'Completed': [f_count, g_count, h_count]
    })
    
    # Store in session state for use by main code
    st.session_state.slab_df = df
    
    # Debug output
    st.write("### Slab Cycle Summary")
    st.write(f"Tower F: {f_count} completed slabs")
    st.write(f"Tower G: {g_count} completed slabs")
    st.write(f"Tower H: {h_count} completed slabs")
