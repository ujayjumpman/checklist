...
# #                         # **HARDCODE: Set Tower 5 Civil Works and Slab Conduting to 78**
# #                         if tower_name == "T5":
# #                             if category == "Civil Works" and activity_name in ["Concreting", "Shuttering", "Reinforcement", "De-shuttering"]:
# #                                 cos_data_dict[key] = {"count": 78, "open_missing": open_missing}
# #                                 logger.info(f"HARDCODED: T5 {activity_name} set to 78")
# #                             elif activity_name == "Slab Conduting":
# #                                 cos_data_dict[key] = {"count": 78, "open_missing": open_missing}
# #                                 logger.info(f"HARDCODED: T5 Slab Conduting set to 78")
...
# #                 if category == "Civil Works" and activity_name in ["Concreting", "Shuttering", "Reinforcement", "De-shuttering"]:
# #                     cos_count = 78
# #                     logger.info(f"HARDCODED: T5 {activity_name} set to 78 (was {cos_data['count']})")
# #                     st.write(f"✓ Hardcoded T5 {activity_name} to 78")
# #                 elif activity_name == "Slab Conduting":
# #                     cos_count = 78
# #                     logger.info(f"HARDCODED: T5 Slab Conduting set to 78 (was {cos_data['count']})")
# #                     st.write(f"✓ Hardcoded T5 Slab Conduting to 78")
...
# #         desired_towers = ['T4A', 'T4B', 'T5', 'T6', 'T7']
...
def generate_consolidated_Checklist_excel(ai_data):
    try:
        ...
        for key in all_keys:
            ...
            # **HARDCODE: Tower 6 Finishing Activities**
            if tower_name == "T6" and activity_name == "Floor Tiling":
                cos_count = 60
                logger.info(f"HARDCODED: T6 Floor Tiling set to 60 (was {cos_data['count']})")
                st.write(f"✓ Hardcoded T6 Floor Tiling to 60")

            if tower_name == "T6" and activity_name == "POP & Gypsum Plaster":
                cos_count = 159
                logger.info(f"HARDCODED: T6 POP & Gypsum Plaster set to 159 (was {cos_data['count']})")
                st.write(f"✓ Hardcoded T6 POP & Gypsum Plaster to 159")

            if tower_name == "T6" and activity_name == "Wall Tiling":
                cos_count = 59
                logger.info(f"HARDCODED: T6 Wall Tiling set to 59 (was {cos_data['count']})")
                st.write(f"✓ Hardcoded T6 Wall Tiling to 59")

            if tower_name == "T6" and activity_name == "Waterproofing - Sunken":
                cos_count = 91
                logger.info(f"HARDCODED: T6 Waterproofing - Sunken set to 91 (was {cos_data['count']})")
                st.write(f"✓ Hardcoded T6 Waterproofing - Sunken to 91")

            # **HARDCODE: Tower 6 MEP Works Activities**
            if tower_name == "T6" and activity_name == "Plumbing Works" and category == "MEP Works":
                cos_count = 168
                logger.info(f"HARDCODED: T6 Plumbing Works (MEP Works) set to 168 (was {cos_data['count']})")
                st.write(f"✓ Hardcoded T6 Plumbing Works to 168")

            if tower_name == "T6" and activity_name == "Wall Conduting" and category == "MEP Works":
                cos_count = 168
                logger.info(f"HARDCODED: T6 Wall Conduting (MEP Works) set to 168 (was {cos_data['count']})")
                st.write(f"✓ Hardcoded T6 Wall Conduting to 168")

            if tower_name == "T6" and activity_name == "Wiring & Switch Socket" and category == "MEP Works":
                cos_count = 0
