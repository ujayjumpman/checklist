...
#             # **NEW: Slab conduting should match Concreting count exactly**
#             if activity == "Slab conduting":
#                 count_table.loc[activity, 'Count_Filtered'] = concreting_count_special
#                 count_table.loc[activity, 'Count_Unfiltered'] = concreting_count_special
#                 logger.info(f"Set Slab conduting count to match Concreting: {concreting_count_special} for {tname}")
#                 st.write(f"**Setting Slab conduting count to match Concreting: {concreting_count_special}**")
#                 continue
...
#                 # **VERIFICATION: Log Slab conduting count**
#                 slab_count = count_table.loc['Slab conduting', 'Count_Filtered']
#                 concreting_count = count_table.loc['Concreting', 'Count_Filtered']
#                 logger.info(f"{tname} - Concreting: {concreting_count}, Slab conduting: {slab_count}")
#                 st.write(f"**Verification: {tname} - Concreting: {concreting_count}, Slab conduting: {slab_count}**")
...
#             # **NEW: Slab conduting should match Concreting count exactly**
#             if activity == "Slab conduting":
#                 slab_count_adjusted = max(concreting_count_special - 5, 0)
#                 count_table.loc[activity, 'Count_Filtered'] = slab_count_adjusted
#                 count_table.loc[activity, 'Count_Unfiltered'] = slab_count_adjusted
#                 logger.info(f"Set Slab conduting count to match Concreting: {slab_count_adjusted} for {tname}")
#                 st.write(f"**Setting Slab conduting count to match Concreting: {slab_count_adjusted}**")
#                 continue
    ...
    def process_tower_data(tower_data, tname):
        ...
        for activity in specific_activities:
            ...
            if activity == "Slab conduting":
                ...
                tower_key = tname if isinstance(tname, str) else str(tname)
                slab_count_adjusted = slab_values.get(tower_key, 0) 
                count_table.loc[activity, 'Count_Filtered'] = slab_count_adjusted
                count_table.loc[activity, 'Count_Unfiltered'] = slab_count_adjusted
                logger.info(f"Set hardcoded Slab conduting count for {tower_key}: {slab_count_adjusted}")
                st.write(f"**Setting hardcoded Slab conduting count for {tower_key}: {slab_count_adjusted}**")
                continue
